...
class SentimentTrade(StrategyBase):
    ...
    def __init__(self):
        ...
        self.signal0 = bt.indicators.CrossOver(self.sma_fast, self.sma_slow)
        ...
        self.signal1 = bt.indicators.CrossOver(self.ema_fast, self.ema_slow)
    ...
    def next(self):
        ...
        if self.p.sig==0:
            if self.p.mode==0:
                ...
                if not self.position and self.signal0 == 1:
                    self.order = self.buy()

                if self.getposition().size < 0 and self.signal0 == 1:
        ...
        if self.p.sig==1:
            if self.p.mode==0:
                ...
                if not self.position and self.signal1 == 1:
                    self.order = self.buy()

                if self.getposition().size < 0 and self.signal1 == 1:
                    self.order = self.close()
                    self.order = self.buy()
