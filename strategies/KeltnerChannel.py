...
class Keltner(bt.Indicator):
    params = dict(ema = 200, atr = 170)
    ...
    def __init__(self) -> None:
        self.l.expo = bt.talib.EMA(self.data.close, timeperiod = self.p.ema)
        self.l.atr = bt.talib.ATR(self.data.high, self.data.low, self.data.close, timeperiod = self.p.atr)
...
class KeltnerChannel(bt.Strategy):
    ...
    def __init__(self):
        self.log("Using Keltner Channel strategy")
        self.ketler = Keltner()
    ...
    def next(self):
        ...
        if not self.position and self.close[0] > self.ketler.upper[0]:
            self.order = self.buy()
        if not self.position and self.close[0] < self.ketler.lower[0]:
            self.order = self.sell()
        
        if self.getposition().size < 0 and self.close[0] > self.ketler.expo[0]:
