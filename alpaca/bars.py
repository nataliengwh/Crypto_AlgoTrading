...
def getdata(coin0, coin1):
    ...
    minute_bar_url_coin0 = config.BARS_URL + f'?timeframe=5T&symbols={coin0}&limit=1000&start={(datetime.today() - timedelta(1)).strftime("%Y-%m-%d")}'
    minute_bar_url_coin1 = config.BARS_URL + f'?timeframe=5T&symbols={coin1}&limit=1000&start={(datetime.today() - timedelta(1)).strftime("%Y-%m-%d")}'
    ...
    return data0[-20:].reset_index(), data1[-20:].reset_index()
